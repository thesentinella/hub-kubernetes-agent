...
async fn build_and_send(cfg: &Config, kube: &KubeClient, hub: &HubClient) -> Result<SnapshotCreated> {
    ...
    let snap = InventorySnapshot {
        ...
        agent: AgentInfo {
            ...
            workload_monitoring_enabled: cfg.workload_monitoring_enabled,
            workload_monitoring_namespaces: cfg.workload_monitoring_namespaces.clone(),
            workload_monitoring_targets: cfg.workload_monitoring_targets.clone(),
        },
