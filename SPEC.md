...
| `ACTION_OPERATOR_EXCLUDED_NAMESPACES` | no | `[]` | Comma-separated list of additional namespaces excluded from action-mode reconciliation. Built-in exclusions already include `kube-system`, `kube-public`, `kube-node-lease`, `sentinella`, `tetragon`, and `openshift-*`. |
...
- To enable Action Mode after install, apply `sentinella-action-operator-lifecycle.yaml`, set `ACTION_OPERATOR_ENABLED=true` in `sentinella-hub-k8s-agent-config`, and restart `daemonset/sentinella-hub-k8s-agent` so the leader reloads the ConfigMap.
